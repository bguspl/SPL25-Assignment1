...
DJSession::DJSession(const std::string &name, bool play_all)
    : session_name(name), library_service(), controller_service(),
      mixing_service(), config_manager(), session_config(), track_titles(),
      play_all(play_all), stats() {
...
int DJSession::load_track_to_controller(const std::string &track_name) {
  ...
  std::cout << "[System] Loading track '" << track_name << "' to controller..."
            << std::endl;
  int result = controller_service.loadTrackToCache(*found_track);
  if (result == 1) {
    stats.cache_hits++;
  }
  if (result == 0) {
...
bool DJSession::load_track_to_mixer_deck(const std::string &track_title) {
  ...
  AudioTrack *track_from_cache =
      controller_service.getTrackFromCache(track_title);
  if (!track_from_cache) {
 ...
 * @note Calls print_session_summary() to display results after playlist
 * completion
 */
void DJSession::simulate_dj_performance() {
  ...
  if (session_config.playlists.empty()) {
    std::cerr
        << "[ERROR] No playlists found in configuration. Aborting session."
        << std::endl;
    return;
  }
  std::cout << "\nStarting DJ performance simulation..." << std::endl;
  std::cout << "BPM Tolerance: " << session_config.bpm_tolerance << " BPM"
            << std::endl;
  std::cout << "Auto Sync: "
            << (session_config.auto_sync ? "enabled" : "disabled") << std::endl;
  std::cout << "Cache Capacity: " << session_config.controller_cache_size
            << " slots (LRU policy)" << std::endl;
  std::cout << "\n--- Processing Tracks ---" << std::endl;

  std::cout << "TODO: Implement the DJ performance simulation workflow here."
            << std::endl;
  // Your implementation here
  if (play_all) {
    std::vector<std::string> playlist_names;
    for (auto &playlist : session_config.playlists) {
...
//   Interactive mode: prompt user to select a playlist
  else {
    bool loaded = false;
    while(true){
    ...
    loaded = load_playlist(selected_playlist);
    if (!loaded) {
...
bool DJSession::load_configuration() {
  ...
  std::cout << "Auto Sync: "
            << (session_config.auto_sync ? "enabled" : "disabled") << std::endl;
  std::cout << "Cache Size: " << session_config.controller_cache_size
            << " slots" << std::endl;
...
std::string DJSession::display_playlist_menu_from_config() {
  ...
  int selection = -1;
  while (true) {
    std::cout << "\nSelect a playlist (1-" << playlist_names.size()
              << ", 0 to cancel): ";
    std::string input;
    ...
    if (ss >> selection && ss.eof()) {
      if (selection == 0) {
        return "";
                 ...
                 selection <= static_cast<int>(playlist_names.size())) {
